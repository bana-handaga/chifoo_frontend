...
// ─── Pipe: Title Case Indonesia ──────────────────────────────────────────────
// Mengonversi ALL CAPS menjadi Title Case standar Indonesia.
// Kata sambung/depan tetap huruf kecil (kecuali posisi awal kalimat).
// Kata ≤ 3 huruf yang sudah terkenal sebagai singkatan dibiarkan uppercase.
const ID_LOWER_WORDS = new Set([
  'dan','atau','yang','di','ke','dari','untuk','pada','oleh','dalam',
  'dengan','antara','tentang','sebagai','bagi','serta','agar','namun',
  'an','bin','binti','the','of','in','and','for','to','at','by',
]);

// Singkatan / akronim yang harus tetap UPPERCASE
const ID_KEEP_UPPER = new Set([
  // Jenjang pendidikan
  ...
  // Lembaga & program
  'PKM','KKN','KKL','KKM','PPM','P2M','LP2M','LPPM','LPPM','DIKTI',
  'BIMA','SIMLITABMAS','DRTPM','DIPA','APBN','APBD',
  'BPJS','BAZNAS','LAZISMU','LAZISNU','BTM',
  // Singkatan umum pemerintah & kesehatan
  'RT','RW','UMKM','KUKM','UKM','IKM','BUMDes','BUMDES',
  'KTP','KK','NIK','NPWP','SIM','STNK','BPKB',
  'MCK','PHBS','CTPS','ODF','STB','ODF','PDAM','PLN','BBM',
  'TB','HIV','AIDS','DBD','ISPA','PMT','ASI','MPASI','IMD','KIA',
  'IGD','ICU','UGD','OK','IRD','BPJS','PKS','MOU','MOA',
  'ILS','BLS','ALS','CPR','AED','AHA',
  // Data & TI
  'IT','ICT','TIK','TI','AI','IoT','CCTV','GPS','GIS',
  'DAPODIK','EMIS','SISTER','SIMPKB','SINTA','PDDikti',
  'WEB','API','APP','SIM','ERP','LMS','CMS','SMS','WA','QR',
  // Kode skema pengabdian
  'KIE','KIE','IbM','IbK','IbW','IbPE','IbDM','IbKIK',
  'PKMIPT','PKPM','PKMK','PKMKC','PKMM','PKMT','PKMPI','PKMRSH',
  'PKMPE','PKMPM','PKMPMD',
  // Akronim umum
  'SD','RA','MAN','MIN','MTSN','SMAN','SMKN',
  'RSUD','RS','RSU','RSIA','RSUP','PUSKESMAS','PKD','POSYANDU',
  'BKD','BKN','BNN','BNNK','BNNP','POLRI','TNI','SAR',
  'BI','OJK','LPS','IAPI','IAI','IDI','PDGI','IBI',
  'NU','PP','PW','PC','PAC','PR','RANTING',
  'AISYIYAH','MUHAMMADIYAH','NASYIATUL','HIZBUL','IPM','IMM','KOKAM',
  // Singkatan kota/wilayah umum
  'DKI','DIY','DI','NTB','NTT','NAD','DKI',
  // Lain
  'SDM','SDA','SDG','MDGs','SPM','ISO','SNI','HACCP','GMP','SOP',
  'MHD','ABCD','CBR','PBL','PAR','RRA','PRA','ZOPP',
]);
...
function toTitleCaseId(text: string): string {
  ...
  return text
    ...
    .filter(w => w.length > 0)
    .map((word, idx) => {
      // Pisahkan prefix (kurung buka) dan suffix (kurung tutup)
      const prefix  = word.match(/^[(\[]+/)?.[0] ?? '';
      const suffix  = word.match(/[)\],:;]+$/)?.[0] ?? '';
      const stripped = word.slice(prefix.length, word.length - suffix.length);
      const upper   = stripped.toUpperCase();

      // Singkatan yang harus tetap uppercase
      if (ID_KEEP_UPPER.has(upper)) return prefix + upper + suffix;
      // Kata sambung/depan → lowercase (kecuali kata pertama)
      if (idx > 0 && ID_LOWER_WORDS.has(stripped)) return word;
      // Kapitalisasi huruf pertama
      return prefix + stripped.charAt(0).toUpperCase() + stripped.slice(1) + suffix;
    })
    .join(' ');
}

@Pipe({ name: 'titleCaseId' })
export class TitleCaseIdPipe implements PipeTransform {
  transform(value: string): string { return toTitleCaseId(value); }
}
...
interface Pengabdian {
  ...
  status:      string;
  sumber:      string;
  authors:     PengabdianAuthor[];
}
interface PengabdianResponse {
  count:     number;
  page:      number;
  page_size: number;
  results:   Pengabdian[];
...
<div class="sp-wrap">
  ...
  <ng-container *ngIf="stats">
    ...
    <div class="sp-cards">
      <div class="sp-card sp-card--blue">
        <div class="sp-card__val">{{ stats.total_pengabdian | number }}</div>
        <div class="sp-card__lbl">Total Kegiatan</div>
        <div class="sp-card__note">judul pengabdian unik</div>
      </div>
      ...
      <div class="sp-accordion__body">
        ...
        <div class="sp-section">
          <div class="sp-section__head">
            <h2 class="sp-section__title">Tren Pengabdian Masyarakat PTMA</h2>
            <span class="sp-section__note">Agregat dari semua dosen yang sudah ter-scrape</span>
          </div>
          <div class="sp-tren-tabs">
            <button class="sp-tren-tab" [class.active]="trenTab==='service'" (click)="trenTab='service'">Jumlah Kegiatan (Author Trend)</button>
            <button class="sp-tren-tab" [class.active]="trenTab==='judul'" (click)="trenTab='judul'">Jumlah Judul Unik</button>
          </div>
          <ng-container *ngIf="trenTab==='service'">
            <ng-container *ngTemplateOutlet="lineChart; context:{data: stats.tren_service, color:'#0284c7', fill:'#e0f2fe'}"></ng-container>
          </ng-container>
        ...
        <div class="sp-grid-2">
          <div class="sp-section">
            <h2 class="sp-section__title">Top Skema Pengabdian</h2>
            <div class="sp-bar-list">
              <div class="sp-bar-row" *ngFor="let s of stats.top_skema">
                ...
                <div class="sp-bar-wrap">
                  <div class="sp-bar sp-bar--blue" [style.width.%]="barPct(s.jumlah, stats.top_skema[0].jumlah)"></div>
                </div>
          ...
          <div class="sp-section">
            ...
            <div class="sp-donut-wrap">
              <svg viewBox="0 0 160 160" width="140" height="140">
                ...
                <text x="80" y="75" text-anchor="middle" font-size="13" font-weight="700" fill="#1e293b">{{ stats.total_pengabdian | number }}</text>
                <text x="80" y="92" text-anchor="middle" font-size="9" fill="#64748b">kegiatan</text>
              </svg>
    ...
    <div class="sp-accordion" [class.sp-accordion--open]="acc2Open">
      <button class="sp-accordion__header" (click)="acc2Open = !acc2Open">
        ...
        <span class="sp-accordion__title">Top 10 Ketua Pengabdian</span>
        <span class="sp-accordion__chevron">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" width="16" height="16"><polyline points="6 9 12 15 18 9"/></svg>
        </span>
      </button>
        ...
        <div class="sp-section">
          <div class="sp-top-list">
            <div class="sp-top-row" *ngFor="let k of stats.top_ketua; let i = index"
              ...
              <span class="sp-top-n">{{ k.jumlah }}<small> keg.</small></span>
  ...
  <div class="sp-accordion" [class.sp-accordion--open]="acc3Open">
    <button class="sp-accordion__header" (click)="acc3Open = !acc3Open">
      ...
      <span class="sp-accordion__title">Daftar Kegiatan Pengabdian</span>
      <span class="sp-accordion__badge" *ngIf="listTotal">{{ listTotal | number }} kegiatan</span>
      <span class="sp-accordion__chevron">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" width="16" height="16"><polyline points="6 9 12 15 18 9"/></svg>
      </span>
    </button>
    ...
    <div class="sp-accordion__body">
          ...
          <thead>
            <tr>
              ...
              <th class="sp-th sp-th--judul sp-th--sort" (click)="toggleSort('judul')">
                Judul Pengabdian <span class="sp-sort-icon" [class.sp-sort-icon--active]="sortField==='judul'">{{ sortIcon('judul') }}</span>
              </th>
      ...
      <div class="sp-pagination" *ngIf="listTotalPages > 1">
        ...
        <span class="sp-page-total">{{ listTotal | number }} kegiatan</span>
      </div>
    ...
    <ng-container *ngIf="authorPopup as p">
      ...
      <div class="ap-section">
        ...
        <table class="ap-table">
          <thead>
            <tr>
              <th>Sumber</th><th>Artikel</th><th>Sitasi</th>
              <th>H</th><th>i10</th><th>G</th>
            </tr>
          </thead>
          <tbody>
            <tr>
              ...
              <td>{{ p.scopus_i10_index }}</td>
              <td>{{ p.scopus_g_index }}</td>
            </tr>
            <tr>
              ...
              <td>{{ p.gscholar_i10_index }}</td>
              <td>{{ p.gscholar_g_index }}</td>
            </tr>
            <tr>
      ...
      <!-- Mini sparkline tren -->
      <div class="ap-section" *ngIf="trenData(p,'scopus').length || trenData(p,'gscholar_pub').length || trenData(p,'service').length || trenData(p,'research').length">
...
`,
  styles: [`
    ...
    .sp-spinner {
      ...
      border: 2px solid #e2e8f0; border-top-color: #0284c7;
    ...
    .sp-card--blue   { border-color: #0284c7; }
    .sp-card--teal   { border-color: #0d9488; }
    .sp-card--green  { border-color: #16a34a; }
    ...
    .sp-accordion__icon { display: flex; color: #0284c7; }
    .sp-accordion__title { flex: 1; text-align: left; }
    .sp-accordion__badge {
      font-size: .72rem; font-weight: 600; color: #0284c7;
      background: #e0f2fe; padding: .15rem .5rem; border-radius: 20px;
    }
    ...
    .sp-tren-tab.active { background: #0284c7; color: #fff; border-color: #0284c7; font-weight: 600; }
    ...
    .sp-rank      { width: 24px; height: 24px; border-radius: 50%; background: #0284c7; color: #fff; font-size: .75rem; font-weight: 700; display: flex; align-items: center; justify-content: center; flex-shrink: 0; }
    .sp-top-info  { flex: 1; min-width: 0; }
    .sp-top-nama  { font-size: .875rem; font-weight: 600; color: #1e293b; }
    .sp-top-n     { font-size: .875rem; font-weight: 700; color: #0284c7; white-space: nowrap; }
    ...
    .sp-filter-input:focus { border-color: #0284c7; box-shadow: 0 0 0 2px rgba(2,132,199,.15); }
    .sp-filter-select {
    ...
    .sp-filter-select:focus { border-color: #0284c7; }
    .sp-filter-reset {
    ...
    .sp-sort-icon--active { color: #0284c7; font-weight: 700; }
    ...
    .sp-tr--expanded { background: #f0f9ff; }
    ...
    .sp-team-count {
      ...
      background: #e0f2fe; color: #0369a1; font-weight: 700; font-size: .75rem;
    }
    ...
    .sp-top-row--clickable:hover { background: #e0f2fe; }
    .sp-top-pt   { font-size: .72rem; color: #2563eb; font-weight: 600; margin-top: .1rem; }
    .sp-top-arrow { color: #94a3b8; font-size: 1rem; margin-left: .25rem; }

    /* ── Author chip clickable ── */
    .sp-author-chip { cursor: pointer; transition: background .12s, color .12s; }
    .sp-author-chip:hover { background: #0284c7; color: #fff; }
    ...
    .ap-close {
      ...
      color: #94a3b8; cursor: pointer; line-height: 1; padding: 2px 6px;
      border-radius: 4px;
    }
    ...
    .ap-pt   { font-size: 12px; font-weight: 700; color: #0284c7; margin-top: 3px; }
    ...
    .ap-table th {
      text-align: left; color: #9ca3af; font-weight: 600; font-size: 10px;
    ...
    .ap-link-sinta {
      display: inline-block; font-size: 12px; color: #0284c7;
      text-decoration: none; font-weight: 600;
      padding: .35rem .9rem; border: 1px solid #bae6fd; border-radius: 6px;
      background: #f0f9ff; transition: background .15s;
    }
    .ap-link-sinta:hover { background: #e0f2fe; }
  `]
})
export class SintaPengabdianComponent implements OnInit, OnDestroy {
  ...
  loadStats() {
    ...
    this.http.get<StatsResponse>(`${API}/sinta-pengabdian/stats/`)
  ...
  loadList(page: number) {
    ...
    this.http.get<PengabdianResponse>(`${API}/sinta-pengabdian/`, { params })
  ...
  // Kembalikan subset titik untuk label X-axis agar tidak terlalu rapat.
  // Maksimal 12 label; selalu tampilkan titik pertama dan terakhir.
  ...
  private readonly DONUT_COLORS = [
    '#0284c7','#0d9488','#16a34a','#ea580c','#7c3aed',
